...
from .multimodal_projector.builder import build_vision_projector, BimodalIterAttn
...
class TALlavaMetaModel:

    def __init__(self, config):
        super(TALlavaMetaModel, self).__init__(config)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        # New args for the vPool layer
        num_learnable_tokens = model_args.num_learnable_tokens
        ...
        # Intialize bimodal iterative attention
        ...
        if getattr(self, "vision_priori", None) is None:
            self.vision_priori = nn.Embedding(
                num_learnable_tokens, self.config.hidden_size
            )
        ...
        if pretrain_mm_mlp_adapter is not None:
            ...
            self.bottle_neck.load_state_dict(get_w(mm_projector_weights, "bottle_neck"))
            self.vision_priori.load_state_dict(
                get_w(mm_projector_weights, "vision_priori")
            )
...
class TALlavaMetaForCausalLM(ABC):
